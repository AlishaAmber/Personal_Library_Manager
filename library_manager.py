...
def create_visualization(status):
    ...
    with st.form(key='add_book_form'):
        ...
        with col2:
            genre = st.selectbox("Genre", [
                "Friction", "Non-Friction", "Science", "Technology", "Fantasy", "Poetry", "Self-Improvement", "Others"
            ])
            ...
            with cols[i % 2]:
                 st.markdown(f"""<div class = 'book-card'>
                            ...
                            <p><span class= '{"read-badge" if book["read_status"] else"unread-badge"}'>{
                                "Read" if book ["read_stats"] else "Unread"
                                }</span></p>
                                </div>
""",unsafe_allow_html=True)
            ...
            with col2:
                    ...
                    if st.button(status_label, key=f"status{i}", use_container_width=True):
                        st.session_state.library[i]['read_status']= new_stats
                        save_library()
                        st.rerun()
...
st.markdown("---")
st.markdown("Copyright @ 2025 Alisha Amber Personal Library Manager", unsafe_allow_html=True)
